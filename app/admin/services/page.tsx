...
export default function ServicesManagement() {
  ...
  const [formData, setFormData] = useState({ title: '', description: '', icon_url: '', image_url: '', sort_order: 0 });
  ...
  const fetchServices = async () => {
    try {
      ...
      if (error) {
        ...
        alert("Failed to load services. See console for details.");
      }
  ...
  const handleOpenModal = (service?: Service) => {
    if (service) {
      ...
      setEditingId(null);
      setFormData({ title: '', description: '', icon_url: '', image_url: '', sort_order: services.length });
    }
    setIsModalOpen(true);
  };
  ...
  const handleCloseModal = () => {
    ...
    setEditingId(null);
    setFormData({ title: '', description: '', icon_url: '', image_url: '', sort_order: 0 });
  };
          ...
          {services.length === 0 ? (
            ...
            services.map((service) => (
              <div key={service.id} className="bg-white overflow-hidden shadow rounded-lg flex flex-col">
                ...
                <div className="p-5 flex-1 mt-4">
                  <h3 className="text-lg font-bold text-gray-900 mb-2">{service.title}</h3>
                  <p className="text-sm text-gray-600 mb-4">{service.description}</p>
                  <p className="text-xs text-gray-400">Sort Order: {service.sort_order}</p>
                </div>
